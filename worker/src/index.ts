...
async function cronAlertCheck(env: Env, scheduledTimeMs: number = Date.now()): Promise<CronResult> {
  ...
  // #500 — persistent (1h+) unreadable-source alert. Independent of the status-alert path above:
  // sweeps the tracking:state blob's failSince fields (#1224) and warns the operator once per service per 24h.
...
export default {
  async scheduled(event: ScheduledEvent, rawEnv: Env, ctx: ExecutionContext): Promise<void> {
    ...
    try {
      ...
      ctx.waitUntil(
        maybeDispatchWorkflow(env, DEEPSEEK_DISPATCH_CONFIG).catch((err) =>
          console.warn('[cron] deepseek dispatch failed:', err instanceof Error ? err.message : err)
        )
      )
      // #1395 — same mechanism, ~hourly cadence (its own cooldown, not */5) — see MISTRAL_DISPATCH_CONFIG.
      ctx.waitUntil(
        maybeDispatchWorkflow(env, MISTRAL_DISPATCH_CONFIG).catch((err) =>
        ...
        if (!summaryMarker) {
          try {
            ...
            await Promise.all(SERVICES.filter(s => s.apiUrl).map(async (svc) => {
              ...
              const reasons = reasonsFor(parseFailDay, svc.id)
