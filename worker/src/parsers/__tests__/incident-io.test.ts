...
describe('computeIncidentIoUptime (#1006)', () => {
  ...
  it('a clean 30-day window is 100%', () => {
    expect(computeIncidentIoUptime(html([], ESTABLISHED), 'c1', NOW)).toEqual({ pct: 100, days: 30, todayWeightedOutageSec: 0, missing: [] })
  })
  ...
  it('a full outage is weighted 1.0 — 24h out of 30 days', () => {
    ...
    expect(out).toEqual({ pct: 96.66, days: 30, todayWeightedOutageSec: 0, missing: [] })
  })
  ...
  it('a young component reports the window it actually covers (#1004 — a page migration resets it)', () => {
    ...
    expect(out).toEqual({ pct: 100, days: 6, todayWeightedOutageSec: 0, missing: [] })
  })
  ...
  it('ids that resolve to nothing are skipped, and the result reflects only what resolved', () => {
    ...
    // exactly the id(s) that did not resolve, distinct from the pct/days/todayWeightedOutageSec computed
    // over the ones that did.
    expect(out).toEqual({ pct: 100, days: 30, todayWeightedOutageSec: 0, missing: ['gone'] })
  })
  ...
  it('OVERLAPPING impacts on one component merge (worst-weight-wins), never sum', () => {
    ...
    expect(out).toEqual({ pct: 99.48, days: 30, todayWeightedOutageSec: 0, missing: [] })
  })
