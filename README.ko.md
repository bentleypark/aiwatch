...
**42개 AI 서비스**의 상태, 지연시간, 가동률, 인시던트를 실시간으로 모니터링하는 대시보드입니다.
...
- **실시간 상태 모니터링** — 42개 AI 서비스의 정상 / 성능 저하 / 장애 상태
- **PWA 지원** — 홈 화면 추가, Service Worker 오프라인 캐시
- **지연시간 측정** — 29개 probe 대상 서비스의 API 엔드포인트 직접 RTT 측정, 나머지는 상태 페이지 응답 시간
...
대시보드 카테고리 분류 기준(총 42개 — 사이드바 필터 / Overview 섹션과 동일).
...
브라우저 (React SPA, 60초 폴링)
  ↓
Cloudflare Worker
  ├── GET /api/status    → 병렬 fetch (42개 서비스) → 정규화
...
Cloudflare KV
  ...
  ├── probe:24h            (헬스체크 프로브, 최대 2016개, TTL 7일, 29개 API 서비스)
...
api/
  ...
  is-down.ts           # Vercel Edge Function — "Is X Down?" SSR 페이지 (40개 서비스)
  is-down/
