...
**39개 AI 서비스**의 상태, 지연시간, 가동률, 인시던트를 실시간으로 모니터링하는 대시보드입니다.
...
- **실시간 상태 모니터링** — 39개 AI 서비스의 정상 / 성능 저하 / 장애 상태
...
브라우저 (React SPA, 60초 폴링)
  ↓
Cloudflare Worker
  ├── GET /api/status    → 병렬 fetch (39개 서비스) → 정규화
...
api/
  ...
  is-down.ts           # Vercel Edge Function — "Is X Down?" SSR 페이지 (34개 서비스)
  is-down/
