const en = {
  ...
  'aboutScore.intro': 'AIWatch Score is a composite reliability metric combining uptime, incident impact days, recovery time, and (for probed API services) responsiveness. Scored out of 100, based on 30-day data.',
  'aboutScore.formula': 'Formula Overview',
  'aboutScore.incidentTableNote': 'Table values assume critical/major impact days (weight 1.0). Minor-only days are weighted at 0.3 — e.g., 5 minor days ≈ 1.5 weighted days ≈ 21.5 score, not 15.2.',
  'aboutScore.whyDays': 'How are affected days counted?',
  'aboutScore.whyDaysDesc': 'Some AI services report separate incidents per model (e.g., Opus/Sonnet/Haiku) — the same outage gets counted multiple times, so we use affected days instead of raw incident count. Each day is weighted by the worst impact of the day: critical/major = 1.0, minor = 0.3, informational/null = excluded. This matches the Atlassian-style weighting used for uptime calculation, so a day of minor advisories no longer over-penalizes the score the way a critical outage does.',
  ...
  'aboutScore.naDesc': 'All services are scored — those with no incidents receive full incident/recovery points, resulting in an estimated score around 86 with medium confidence.',
  'aboutScore.rankExclude': 'Ranking Exclusion',
  'aboutScore.rankExcludeDesc': 'Services are excluded from the Reliability Ranking when insufficient data is available for a fair comparison. Specifically, services with only estimated uptime (derived from incident durations) and no impactful incidents — none recorded, or only informational notices — lack the observable data points needed to produce a meaningful score. Official uptime alone is not the criterion — even services without published uptime can be ranked if they have enough incident history to evaluate.',
  ...
  'aboutScore.responsivenessNA2': 'Their score is rescaled from the 80-point base to 100 with no penalty.',
  'aboutScore.responsivenessInsufficient': 'Newly added probed services receive a 5% penalty until 7 days of probe data accumulate.',
