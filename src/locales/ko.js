const ko = {
  ...
  'incidents.duration.unknown': '기간 미상',
  ...
  'incidents.startUnknown.note': '제공사 기록에는 이 인시던트의 해결 시각이 시작 시각보다 앞서 있고, 실제 구간을 복구할 영향 기록을 확보하지 못했습니다. 표시된 시각은 제공사가 발행한 값이지만 시작인지 종료인지 확정할 수 없고, 지속 시간도 알 수 없습니다.',
  'incidents.zeroLengthRecord.note': '이 인시던트의 출처 기록은 시작과 종료를 같은 하나의 시각으로 담고 있어, 지속 시간을 도출할 수 없습니다. 표시된 시각이 그 시각입니다.',
  ...
  // #1268 — sourceDead.body / sourceDead.bodyProbe 쌍과 같은 형태의 프로브 근거 변형. 초록 배지가
  // 누구의 근거에 기대고 있는지를 밝힌다 — 제공자의 상태 페이지가 아니라 우리의 직접 측정.
  // #1268 round 7 — 장애로 표시되는 상태용 변형. 나머지 둘은 "장애가 있다는 뜻이 아니다"로 끝나는데,
  // 빨강/노랑 pill 옆에서는 거짓이다(프로브가 확증한 장애, 릴레이 지연 중 live down 배지). 그렇다고
  // 노트를 통째로 감추면 업타임·인시던트·MTTR·점수·캘린더가 전부 빈 채 설명이 없어진다. 덜 말하되
  // 침묵하지는 않는다. 생략 사실만 주장한다: "위 상태는 마지막으로 읽어낸 값"이라는 문장을 붙였다가
  // 뺐는데, 프로브가 확증한 판정은 만들어진 것이라(실패 3회 → Phase 1 승격) 그 뒤에 '마지막 읽기'가
  // 아예 없기 때문이다.
  'svc.sourceUnknown.bodyAffected': 'AIWatch가 이 서비스의 공식 상태 페이지를 현재 읽지 못하고 있어 업타임·점수·랭킹은 출처가 복구될 때까지 표시하지 않습니다.',
  'svc.sourceUnknown.bodyProbe': 'AIWatch가 이 서비스의 공식 상태 페이지를 읽지 못하고 있지만, AIWatch가 직접 호출한 결과 서비스가 정상 응답합니다 — 제공자 발표가 아니라 우리 측정에 근거해 운영 중으로 표시합니다. 업타임·인시던트는 출처를 다시 읽을 수 있게 되면 제공됩니다.',
