...
| `outbound_fallback_click` | `from_service`, `to_service`, `location` (`is_down_page` / `action_banner` / `analyze_modal`) | Is X Down SSR (`renderFallbacks` "Open ↗") · Overview ActionBanner · Analyze modal (both SPA, `outboundReferralUrl`) | #842 — disclosed OUTBOUND click to the recommended alternative's provider site (the Rung-1 referral evidence, #637). Edge fires from the delegated `[data-ga]` listener; the SPA surfaces fire `trackEvent` + `sendReferralBeacon` on the `↗` link. The link is `rel="nofollow"` (unpaid editorial — `sponsored` intentionally omitted so it doesn't contradict the "not paid" disclosure; add it only if a service becomes a paid sponsor), only rendered for services with a curated `SERVICE_SITE_URL` (Edge slug-map ↔ SPA constants, parity-pinned). GA is the consent-gated floor — the consent-free server counter (`POST /api/referral` → `referral:out:{date}` → daily summary) is the honest number |
