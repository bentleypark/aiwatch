...
| `copy_badge` | `location`, `service_id` | Is X Down (Embed badge section) | Embeddable status-badge markdown copied (#805 Problem B). `location`=`is_down_page`; `service_id`=the worker service id. Measures badge-embed demand (the SEO backlink lever) at the high-traffic outage moment |
