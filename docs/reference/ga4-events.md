...
| `report_issue` | `location`, `service_id`, `category` | Is X Down (Report an issue modal) | Crowd report submitted (#575). `category` ∈ `outage`/`degraded`/`errors`/`login`/`other`. Fires on a successful POST to `/api/report-issue` |
