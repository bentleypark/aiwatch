...
A cron-only operator alert (webhook only, never per-user), sibling to the #135 component-**miss** alert but the **inverse direction**: #135 fires when a *configured* id disappears (migration); this fires when an *unseen* id **appears** (a provider added a component AIWatch should decide whether to track). Data is free — `fetchAllServices` returns `pageComponents` (apiUrl → `{id,name}[]`) harvested from the live prefetch; the cron diffs each page against the durable `component-seen:{apiUrl}` KV snapshot (`diffPageComponents`). **First sight of a page bootstraps silently** (records ids, alerts nothing) so a rich shared page (status.openai.com) never dumps its existing components as "new"; thereafter each genuinely-new id → **one** 🆕 heads-up, ever (the snapshot unions it, so it never re-fires; a removed-then-re-added component stays silent). The body (`formatNewComponentAlert`) lists the new component name+id and the AIWatch services on that page; for a `displayAllComponents` page (cerebras) it says *already auto-tracked — heads-up only*, otherwise it points to the `statusComponentIds`/`displayComponentIds` config to edit. The snapshot is persisted **only after a successful send**, so a Discord failure retries next cron rather than dropping the one-shot alert. A fresh-cache cron cycle (no live fetch) leaves `pageComponents` empty and simply skips — detection runs on the next stale-triggered fetch. Pure `diffPageComponents` + `formatNewComponentAlert` unit-tested in `utils.test.ts`.
