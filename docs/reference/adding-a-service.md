...
- **For an incident.io page, check `display_uptime_mode` before concluding "no uptime".** `'chart_and_percentage'` publishes a number; `'chart_only'` emits `"uptime":"$undefined"` and genuinely has none (Stability/ElevenLabs). Replicate moved to the dedicated Cloudflare Status v3 path in #1384.
