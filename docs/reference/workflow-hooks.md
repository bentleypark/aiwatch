...
description: "The .claude/hooks workflow hooks plus the .githooks/pre-push agent gate: what each fires on, hard vs soft, the audit log, and how to monitor + tune the step-3.5 hard gate."
...
when `git merge-base HEAD origin/main` succeeds and the whole branch contains no watched frontend or
Edge paths; a missing base ref, failed Git command, or unexpected environment continues the build.
Vercel uses shallow clones, so the first production verification must record whether its base ref is
available: an unavailable base is safe but sacrifices the skip optimization.

`.claude/settings.json` wires the workflow hooks below, because written rules alone (CLAUDE.md + auto-memory) are passive context loaded once per session, which compaction drops: probabilistic compliance. A hook fires at the decision moment.
- **`workflow-gates-reminder.sh`** (UserPromptSubmit) — re-injects the non-negotiable gates (a start-of-work gate 0 = "invoke `ship-issue` first on any new issue/code task", then the 4 decision-moment gates; text in `.claude/hooks/workflow-gates.txt`) as `additionalContext` on **every turn**. Gate 0 closes the coverage gap where "invoke ship-issue at the START" lived only in CLAUDE.md (a passive session-start surface) and was the one workflow step with no every-turn backstop. Targets the root cause (rules are passive context loaded once at session start; compaction drops methodology), and is the only hook surface that fires each turn + survives compaction. Soft (cannot block); logged as `inject`. Mirrors memory note `feedback_workflow_gates`. **#1412 — gate 2 names the reviewer the loop spawns (`review-findings-only`)**, not the plugin command.
...
- **`tooling-trigger.sh`** (PreToolUse/Edit|Write|MultiEdit) — soft path-based reminder (`hookSpecificOutput.additionalContext`, never blocks) to fetch the **live upstream** first on status-source files, **current docs** (first-party skill, else context7) on SDK/binding files, or **modern-web-guidance** first on frontend HTML/CSS/JS; the #415 backstop for those tools' probabilistic triggers. Logged as `inject`. Trigger map: **[reference-tooling.md](reference-tooling.md)**.
...
- **`public-issue-figures-trigger.sh`** (PreToolUse/Bash, **#1354**) — soft reminder before a listed `gh` write subcommand (`.claude/hooks/public-issue-write-commands.txt`): absolute adoption numbers belong in the private `aiwatch-wiki` bundle, a public issue carries the verdict, field name or ratio. Matches the command NAME and never reads the body — see the section below for why, and for why its effectiveness cannot be measured. Logged as `warn`.
...
that agent is on record, or `AGENT_VERIFIED=1` is set. It is not one of the hooks above: this is a
...
## The public-issue figures reminder, and why its effectiveness is unmeasurable (#1354)

`public-issue-figures-trigger.sh` emits one soft line before a `gh` subcommand listed in
`.claude/hooks/public-issue-write-commands.txt`: absolute adoption numbers belong in the private `aiwatch-wiki` bundle, a public issue carries
the verdict, field name or ratio.

**It never blocks, so it can never produce the signal this document defines as effectiveness.**
`Violations intercepted` counts a `block`, a step-3.5 `deny`, or a `no_verify=1` note — a hook that only
ever logs `warn` contributes to none of them, and its `warn` count is preventive telemetry that scales
with how much issue work a week contained. There is no number that says whether it worked. Do not report
it as enforcement, and do not read a rising or falling `warn` trend as a result.

Two consequences of the design, both accepted rather than solved:

- **It fires on writes that carry no figure**, which is most of them. That noise is the price of not
  reading the body: deciding from prose whether a number is an adoption figure means parsing unbounded,
  externally-shaped input to reach a verdict, which is the failure mode #1348 recorded — every round
  patches one more hole. The hook matches a command name and hands the judgement back.
- **A leak it fails to prevent leaves no trace here.** The counterpart failure — a figure that shipped
  anyway — is visible only by reading the issue, which is what #1354's own scan did.

`scripts/public-issue-figures-trigger.test.mjs` pins its behaviour and its output channel is pinned by
the four-hook roster in `scripts/soft-hook-channel.test.mjs`.
...
The four PreToolUse soft reminders (`git-mutation-gate.sh`, `tooling-trigger.sh`,
`korean-copy-trigger.sh`, and `public-issue-figures-trigger.sh`) deliver their text through
