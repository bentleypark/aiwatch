...
| Endpoint | Notes |
|---|---|
| `GET /api/status` | Live parallel fetch of all 36 services, normalize, KV write, platform/probe cross-validation. Multi-component services carry a `components: {id,name,status}[]` snapshot for the per-component breakdown — from `statusComponentIds` (#604: cerebras/cursor/runway/langsmith/copilot/windsurf) or the display-only `displayComponentIds` (#606: elevenlabs/replicate); absent otherwise |
