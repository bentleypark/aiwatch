...
| `GET /api/report-feed?svc=:id` | — (CORS, `max-age=30`) | Recent 24h crowd reports for one service (#575). Fetched by the is-down Edge ONLY when an independent signal already shows a problem (gate at the call site), so a public list never contradicts an `operational` page |
