...
<section class="section" id="status">
  ...
  <div class="chain">
    ...
    <div class="chain-step">
      ...
      <div class="cs-body" data-i18n="s2.4.body">제목 기반 제외 패턴에 걸리더라도, 인시던트의 컴포넌트 태그가 해당 서비스의 주요 컴포넌트로 시작하거나 인시던트의 컴포넌트 ID가 해당 서비스의 배지 컴포넌트에 포함되면 포함합니다. 제공사가 우리가 보는 컴포넌트에 직접 태깅했다는 것은 영향을 받는다는 제공사 자신의 선언이므로, 제목 문자열 매칭보다 우선합니다. (컴포넌트 이름이 서로 겹칠 수 있어 — 예: 한 페이지에 "Login"이 둘 — ID 대조가 필요합니다.)</div>
    </div>
    <div class="chain-step">
      ...
      <div class="cs-body" data-i18n="s2.5.body">제공사의 상태 페이지는 제품·기능별 컴포넌트로 나뉩니다. 예를 들어 status.claude.com 한 페이지에 Claude API, Claude Code, claude.ai, Cowork가 각각 따로 표시되고, AIWatch는 서비스마다 이 중 어떤 컴포넌트를 볼지 정해두고 있습니다. 제공사가 인시던트 하나를 모든 컴포넌트에 연결해두는 일이 있습니다. 실제로 문제가 난 건 그중 하나뿐인데도 그렇습니다. 이때 우리가 보는 컴포넌트가 정상으로 표시돼 있으면 그 인시던트는 해당 서비스에 표시하지 않습니다. 문제가 난 건 다른 컴포넌트이기 때문입니다. 이는 진행 중인 인시던트에만 적용됩니다 — 이미 해결된 인시던트는 우리 컴포넌트에 연결돼 있었다면 이력으로 남습니다. 한 가지 예외가 있습니다. 우리 컴포넌트가 정상인 것은 앞과 같지만, 제공사가 그 인시던트의 영향도를 &#39;없음(none)&#39;으로 매긴 경우입니다. 이는 어느 컴포넌트도 장애로 내려가지 않았다는 뜻이라, 우리 컴포넌트가 정상인 것도 당연한 결과일 뿐, 이 인시던트가 다른 컴포넌트의 문제라는 근거가 되지 못합니다. 그래서 이때는 상태 대신 &#39;이 인시던트가 우리 컴포넌트에 연결돼 있는가&#39;로 판단해, 연결돼 있으면 유지합니다. Claude API 서비스에는 반대 방향도 적용됩니다. Claude Code에만 영향을 준 장애는 Claude API에 표시되지 않습니다 — Claude API가 마침 다른 문제로 장애 중이더라도 마찬가지입니다.</div>
    </div>
    <div class="chain-step">
...
<section class="section" id="latency">
  ...
  <div class="limits">
    ...
    <p data-i18n="s5.limit.probe"><strong>Probe 미적용:</strong> 레이턴시 랭킹은 직접 probe하는 API 서비스와 자체 API를 가진 코딩 에이전트(예: Cursor)를 대상으로 합니다 — 앱(Character.AI는 probe하되 상세 페이지에만 표시), 자체 API가 없는 코딩 에이전트, probe하지 않는 나머지 3개 AI 서비스는 제외됩니다.</p>
  </div>
</section>
...
const i18n = {
  ko: {
    ...
    's2.4.tag': 'incidentExclude bypass', 's2.4.title': 'incidentExclude 컴포넌트 우회', 's2.4.body': '제목 기반 제외 패턴에 걸리더라도, 인시던트의 컴포넌트 태그가 해당 서비스의 주요 컴포넌트로 시작하거나 인시던트의 컴포넌트 ID가 해당 서비스의 배지 컴포넌트에 포함되면 포함합니다. 제공사가 우리가 보는 컴포넌트에 직접 태깅했다는 것은 영향을 받는다는 제공사 자신의 선언이므로, 제목 문자열 매칭보다 우선합니다. (컴포넌트 이름이 서로 겹칠 수 있어 — 예: 한 페이지에 "Login"이 둘 — ID 대조가 필요합니다.)',
    's2.5.tag': 'component-status filter', 's2.5.title': '컴포넌트 상태 인시던트 필터', 's2.5.body': '제공사의 상태 페이지는 제품·기능별 컴포넌트로 나뉩니다. 예를 들어 status.claude.com 한 페이지에 Claude API, Claude Code, claude.ai, Cowork가 각각 따로 표시되고, AIWatch는 서비스마다 이 중 어떤 컴포넌트를 볼지 정해두고 있습니다. 제공사가 인시던트 하나를 모든 컴포넌트에 연결해두는 일이 있습니다. 실제로 문제가 난 건 그중 하나뿐인데도 그렇습니다. 이때 우리가 보는 컴포넌트가 정상으로 표시돼 있으면 그 인시던트는 해당 서비스에 표시하지 않습니다. 문제가 난 건 다른 컴포넌트이기 때문입니다. 이는 진행 중인 인시던트에만 적용됩니다 — 이미 해결된 인시던트는 우리 컴포넌트에 연결돼 있었다면 이력으로 남습니다. 한 가지 예외가 있습니다. 우리 컴포넌트가 정상인 것은 앞과 같지만, 제공사가 그 인시던트의 영향도를 &#39;없음(none)&#39;으로 매긴 경우입니다. 이는 어느 컴포넌트도 장애로 내려가지 않았다는 뜻이라, 우리 컴포넌트가 정상인 것도 당연한 결과일 뿐, 이 인시던트가 다른 컴포넌트의 문제라는 근거가 되지 못합니다. 그래서 이때는 상태 대신 &#39;이 인시던트가 우리 컴포넌트에 연결돼 있는가&#39;로 판단해, 연결돼 있으면 유지합니다. Claude API 서비스에는 반대 방향도 적용됩니다. Claude Code에만 영향을 준 장애는 Claude API에 표시되지 않습니다 — Claude API가 마침 다른 문제로 장애 중이더라도 마찬가지입니다.',
    ...
    's3.official': 'Official', 's3.officialDesc': '— 제공사 상태 페이지가 공개한 <strong>일별 장애 기록·장애 구간</strong>으로 AIWatch가 <strong>최근 30일치</strong>를 직접 계산합니다. 우리 숫자가 제공사 페이지의 숫자와 다를 수 있습니다. <strong>감추지 않습니다</strong> — 제공사가 발표한 값이 우리 계산과 다르면 서비스 상세 페이지에 <strong>그 값을 집계 기간과 함께 나란히</strong> 보여드립니다. 상태 페이지에 uptime %를 공개하지 않는 서비스(ElevenLabs · Replicate · Stability)는 비교할 값이 없어 우리 계산값만 보여줍니다. 제공사 기록이 30일에 못 미치면(상태 페이지를 옮긴 경우 등) 실제 며칠치인지 밝힙니다.',
  ...
  en: {
    ...
    's2.4.tag': 'incidentExclude bypass', 's2.4.title': 'incidentExclude component bypass', 's2.4.body': 'Even when a title-based exclude pattern matches, the incident is kept if its component tag starts with the service\\\'s primary component, or if its component IDs are among the service\\\'s badge components. The provider tagging an incident onto a component we watch is the provider\\\'s own assertion that it affects us, so it outranks title substring matching. (Component names can collide — one page carries two named "Login" — so IDs are what settle it.)',
    's2.5.tag': 'component-status filter', 's2.5.title': 'Component-status incident filter', 's2.5.body': 'The status page of a provider is split into per-product components. status.claude.com, for example, lists Claude API, Claude Code, claude.ai and Cowork separately, and AIWatch decides in advance which of those components each service follows. Providers sometimes attach a single incident to every one of those components, even when only one of them is actually affected. When the component we watch is shown as operational, we do not show that incident on the service: whatever broke, it was a different component. This applies to incidents still in progress; one that has already been resolved stays as history when it was attached to our component. There is one exception. The component we watch is operational just the same, but the provider rated the incident as "none" impact — meaning nothing went down anywhere, so our component being operational is merely the default and is no evidence that the incident belongs to a different component. There we judge by whether the incident is attached to our component rather than by status, and keep it if it is. On Claude API the reverse also applies: an outage affecting only Claude Code does not appear there, even when Claude API happens to be degraded by something else at the same time.',
