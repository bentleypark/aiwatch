...
<section class="section" id="status">
  ...
  <p class="lead" data-i18n="s2.lead">서비스별 상태는 계층화된 우선순위 체인으로 결정되며, 화면에는 <strong>Operational · Partial · Degraded · Down · Unknown</strong> 로 표기됩니다. Unknown은 서비스에 장애가 있다는 뜻이 아니라, AIWatch가 해당 서비스의 공식 상태 페이지를 읽지 못해 확인할 수 없다는 뜻입니다. 규칙은 위에서부터 순서대로 확인하며, 처음 일치하는 규칙에서 상태가 결정됩니다.</p>
  <div class="chain">
...
const i18n = {
  ko: {
    ...
    's1.src.atlassian': '— 17개 서비스', 's1.src.incidentio': '— 12개 서비스, 컴포넌트 단위 인시던트 + 영향도', 's1.src.betterstack': '— 5개 서비스, 인시던트 RSS + 가동률 JSON', 's1.src.instatus': '— 1개 서비스, 컴포넌트 상태 + 가동률', 's1.src.rootly': '— 1개 서비스: Mistral의 인시던트와 일별 장애 구간을 <strong>브라우저로 읽어</strong> 수집', 's1.src.cloudflare': '— 1개 서비스: Replicate의 정확한 컴포넌트 상태와 귀속된 인시던트(진행 중 + 해결됨)', 's1.src.gcloud': '— 1개 서비스: Gemini API', 's1.src.onlineornot': '— 1개 서비스: OpenRouter 컴포넌트 상태 + 이력', 's1.src.flashduty': '— 2개 서비스: DeepSeek API · DeepSeek App 상태 피드 정규화', 's1.src.awshealth': '— 1개 서비스: Amazon Bedrock 공개 이벤트 JSON API', 's1.src.rss': '— 3개 서비스: Azure Status (Azure OpenAI) · xAI (status.x.ai) · Grok', 's1.src.probe': '— 33개 AI 서비스의 엔드포인트 직접 측정',
    ...
    's2.title': '상태 결정',
    's2.lead': '서비스별 상태는 계층화된 우선순위 체인으로 결정되며, 화면에는 <strong>Operational · Partial · Degraded · Down · Unknown</strong> 로 표기됩니다. Unknown은 서비스에 장애가 있다는 뜻이 아니라, AIWatch가 해당 서비스의 공식 상태 페이지를 읽지 못해 확인할 수 없다는 뜻입니다. 규칙은 위에서부터 순서대로 확인하며, 처음 일치하는 규칙에서 상태가 결정됩니다.',
  ...
  en: {
    ...
    's2.lead': 'Per-service status is resolved by a layered priority chain and shown as <strong>Operational · Partial · Degraded · Down · Unknown</strong>. Unknown does not mean the service is impaired — it means AIWatch could not read the official status page for that provider, so it cannot confirm the status either way. Rules apply top-to-bottom and stop at the first match.',
