...
<section class="section" id="status">
  ...
  <p class="note" data-i18n="s2.partial"><strong>Partial</strong>은 다중 컴포넌트 서비스(Better Stack 기반 — Together · Fireworks · HuggingFace · Modal · Luma · Helicone)에서 전체 서비스는 정상이지만 일부 컴포넌트(예: 특정 모델)만 영향받은 중간 상태입니다. 서비스 전체를 'degraded'로 격상시키지는 않되, 영향받은 컴포넌트의 실제 장애는 uptime · 인시던트 집계를 통해 AIWatch Score · 랭킹에 그대로 반영됩니다.</p>
...
<section class="section" id="uptime">
  ...
  <p class="lead" data-i18n="s3.lead">Uptime은 <strong>제공사가 공개한 장애 기록을 AIWatch가 직접 30일치로 계산</strong>합니다. 제공사가 자기 페이지에 표시하는 값을 그대로 쓰지는 않습니다.</p>
  ...
  <ul>
    ...
    <li><strong data-i18n="s3.platform">Platform</strong> <span data-i18n="s3.platformDesc">— <strong>근거도, 집계 방식도 다릅니다</strong> (Together · Fireworks · HuggingFace · Modal · Luma · Helicone). 이 6개 서비스의 상태 페이지는 Better Stack이 운영하며, 장애 기록이 <strong>제공사의 인시던트 선언이 아니라 Better Stack 모니터가 직접 측정한 다운타임</strong>입니다. 여기에는 아래의 1.0 / 0.3 가중을 적용하지 않고, <strong>모니터가 실제로 관측한 날만</strong> 계산에 넣습니다 — 그래서 한 서비스의 값에 7일치 모니터와 30일치 모니터가 함께 평균될 수 있습니다. 제공사가 보증하는 SLA가 아니라는 뜻이기도 해서 라벨을 구분합니다.</span></li>
  </ul>
  <p data-i18n="s3.grouped">일부 제공사는 여러 컴포넌트를 하나의 섹션으로 묶어 그 섹션 전체의 uptime%를 별도로 발표합니다 (예: DeepSeek의 "对话服务(Chat Service)"는 Instant/Expert/Vision Mode·File Upload·Search 5개 컴포넌트를 묶은 값입니다). 서비스의 범위가 그 섹션 전체와 정확히 일치하면 AIWatch는 제공사가 발표한 섹션 값을 그대로 사용하고, 섹션이 없거나 일부 컴포넌트만 해당하면 개별 컴포넌트 값 중 가장 낮은 값을 사용합니다.</p>
...
const i18n = {
  ko: {
    ...
    's2.partial': '<strong>Partial</strong>은 다중 컴포넌트 서비스(Better Stack 기반 — Together · Fireworks · HuggingFace · Modal · Luma · Helicone)에서 전체 서비스는 정상이지만 일부 컴포넌트(예: 특정 모델)만 영향받은 중간 상태입니다. 서비스 전체를 \\'degraded\\'로 격상시키지는 않되, 영향받은 컴포넌트의 실제 장애는 uptime · 인시던트 집계를 통해 AIWatch Score · 랭킹에 그대로 반영됩니다.',
    ...
    's3.platform': 'Platform', 's3.platformDesc': '— <strong>근거도, 집계 방식도 다릅니다</strong> (Together · Fireworks · HuggingFace · Modal · Luma · Helicone). 이 6개 서비스의 상태 페이지는 Better Stack이 운영하며, 장애 기록이 <strong>제공사의 인시던트 선언이 아니라 Better Stack 모니터가 직접 측정한 다운타임</strong>입니다. 여기에는 아래의 1.0 / 0.3 가중을 적용하지 않고, <strong>모니터가 실제로 관측한 날만</strong> 계산에 넣습니다 — 그래서 한 서비스의 값에 7일치 모니터와 30일치 모니터가 함께 평균될 수 있습니다. 제공사가 보증하는 SLA가 아니라는 뜻이기도 해서 라벨을 구분합니다.',
  ...
  en: {
    ...
    's2.partial': '<strong>Partial</strong> is an intermediate state for multi-component services (Better Stack — Together · Fireworks · HuggingFace · Modal · Luma · Helicone) where the overall service is operational but some components (e.g. a specific model) report issues. It does not escalate the whole service to \\'degraded\\', but the affected component\\'s real outage is still reflected in the AIWatch Score &amp; ranking through the uptime &amp; incident aggregation.',
