...
${CONSENT_INIT_SCRIPT}
<meta name="description" content="Transparent, independent measurement of AI service reliability. How AIWatch determines status, computes uptime and the AIWatch Score, and — explicitly — what we can't measure and why. 40 services, polled every 5 min, UTC.">
...
<header class="doc-header">
  <div class="doc-header-inner">
    ...
    <p class="meta-line" data-i18n="hero.meta">40개 서비스 · 5분 간격 폴링 · UTC 기준</p>
...
<section class="section" id="sources">
  ...
  <p class="lead" data-i18n="s1.lead">AIWatch는 LLM API 15개, 코딩 에이전트 6개, 음성 3개, 추론·인프라 5개, 관측 3개, 영상 2개, 이미지 2개, AI 앱 4개 — 총 40개 AI 서비스를 최대 5분 간격으로 폴링합니다. 모든 시각은 UTC 기준입니다.</p>
  ...
  <ul>
    ...
    <li><strong>Direct RTT probes</strong> <span data-i18n="s1.src.probe">— 27개 AI 서비스의 API 엔드포인트 직접 측정</span></li>
  </ul>
...
<section class="section" id="latency">
  ...
  <p class="lead" data-i18n="s5.lead">27개 AI 서비스의 API 엔드포인트를 Cloudflare Workers 엣지에서 5분 간격으로 직접 측정합니다. p50 / p75 / p95 분위수를 산출합니다.</p>
  <div class="limits">
    ...
    <p data-i18n="s5.limit.probe"><strong>Probe 미적용:</strong> 레이턴시 랭킹은 직접 probe하는 27개 AI 서비스만 대상입니다 — 앱·코딩 에이전트, 그리고 probe하지 않는 나머지 3개 AI 서비스는 제외됩니다.</p>
  </div>
</section>
...
<section class="section" id="score">
  ...
  <div class="subscore">
    ...
    <p data-i18n="s4.resp.desc">5분 간격 health-check probe로 실제 API 엔드포인트의 응답 속도와 안정성을 측정합니다(27개 AI 서비스). 응답 속도와 일관성을 함께 반영합니다.</p>
    ...
    <p data-i18n="s4.resp.na1">앱과 코딩 에이전트는 측정할 API 엔드포인트가 없어 probe 대상이 아니며, probe 세트(27개)에 들지 않은 나머지 3개 AI 서비스(Bedrock · Azure OpenAI · Modal)도 probe하지 않습니다.</p>
...
const i18n = {
  ko: {
    ...
    'hero.meta': '40개 서비스 · 5분 간격 폴링 · UTC 기준',
    'hero.principle': '<strong>측정할 수 있는 것은 공개하고, 측정할 수 없는 것은 분명히 밝힙니다.</strong>',
    's1.title': '측정 대상',
    's1.lead': 'AIWatch는 LLM API 15개, 코딩 에이전트 6개, 음성 3개, 추론·인프라 5개, 관측 3개, 영상 2개, 이미지 2개, AI 앱 4개 — 총 40개 AI 서비스를 최대 5분 간격으로 폴링합니다. 모든 시각은 UTC 기준입니다.',
    ...
    's4.resp.desc': '5분 간격 health-check probe로 실제 API 엔드포인트의 응답 속도와 안정성을 측정합니다(27개 AI 서비스). 응답 속도와 일관성을 함께 반영합니다.',
    's4.resp.speed': 'Speed (0~10) — p50 RTT 지수 감쇠',
    's4.resp.stability': 'Stability (0~10) — 결합 변동계수 지수 감쇠',
    's4.resp.na1': '앱과 코딩 에이전트는 측정할 API 엔드포인트가 없어 probe 대상이 아니며, probe 세트(27개)에 들지 않은 나머지 3개 AI 서비스(Bedrock · Azure OpenAI · Modal)도 probe하지 않습니다.',
    ...
    's5.lead': '27개 AI 서비스의 API 엔드포인트를 Cloudflare Workers 엣지에서 5분 간격으로 직접 측정합니다. p50 / p75 / p95 분위수를 산출합니다.',
    ...
    's5.limit.probe': '<strong>Probe 미적용:</strong> 레이턴시 랭킹은 직접 probe하는 27개 AI 서비스만 대상입니다 — 앱·코딩 에이전트, 그리고 probe하지 않는 나머지 3개 AI 서비스는 제외됩니다.',
  ...
  en: {
    ...
    'hero.meta': '40 services · polled every 5 min · UTC',
    'hero.principle': '<strong>We publish what we can measure — and are explicit about what we can\\\'t.</strong>',
    's1.title': 'What we measure',
    's1.lead': 'AIWatch polls 40 AI services — 15 LLM APIs, 6 coding agents, 3 voice, 5 inference & infra, 3 observability, 2 video, 2 image, and 4 AI apps — up to every 5 minutes. All timestamps are in UTC.',
    ...
    's4.resp.title': 'Responsiveness Score (0–20)',
    's4.resp.desc': 'Measures actual API endpoint speed and stability via 5-minute health-check probes (27 AI services). Combines response speed and consistency.',
    ...
    's4.resp.na1': 'Apps and coding agents have no API endpoint to measure; and 3 other AI services outside the 27-service probe set (Bedrock, Azure OpenAI, Modal) are not probed either.',
    ...
    's5.lead': 'We measure the API endpoints of 27 AI services directly from the Cloudflare Workers edge every 5 minutes, producing p50 / p75 / p95 percentiles.',
    ...
    's5.limit.probe': '<strong>No probe:</strong> The latency ranking covers only the 27 directly-probed AI services — apps, coding agents, and 3 other non-probed AI services are excluded.',
