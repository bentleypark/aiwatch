        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', width: '100%', height: '100%', background: '#080c10', padding: '40px' }}>
          ...
          {(score || uptime) && (
            <div style={{ display: 'flex', marginTop: '28px', fontSize: '20px', color: '#8b949e' }}>
              {score && <div style={{ display: 'flex', marginRight: '40px' }}>{`Score: `}<span style={{ color: '#e6edf3', fontWeight: 600, marginLeft: '6px' }}>{score}</span></div>}
              {uptime && <div style={{ display: 'flex' }}>{`Uptime: `}<span style={{ color: '#e6edf3', fontWeight: 600, marginLeft: '6px' }}>{`${uptime}%`}</span></div>}
            </div>
          )}
