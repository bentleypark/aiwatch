...
- **CSP (#482)**: `vercel.json` ships a **`Content-Security-Policy-Report-Only`** header (+ `Reporting-Endpoints`) over SPA + Edge SSR; violations land at the `api/csp-report.ts` Edge sink (Vercel logs). Phase 1 = report-only (`script-src` omits `'unsafe-inline'`). **Phase 2 (in progress, still Report-Only)**: each Edge SSR page sets its OWN per-surface CSP header + refactors every inline `on*=` handler → delegated `addEventListener`, split by cacheability — **nonce** (`api/_shared/csp-nonce.ts`, `no-store`) for low-traffic badges/methodology/intro (#823/#828), **hash** (`api/_shared/csp-hash.ts` `cspForHtml`) for high-traffic **is-down** so it keeps its `s-maxage=60` edge cache (a content hash survives caching; a random nonce doesn't — a cached page would reuse one nonce for all visitors). Phase 3 flips each surface to enforcing + scopes the `vercel.json` `/(.*)` CSP to exclude the migrated Edge routes (else the intersection of co-applied policies blocks the scripts). Full policy/rationale/roadmap: **[docs/reference/reference-csp.md](docs/reference/reference-csp.md)** — read before changing the policy or adding an inline script/external origin
