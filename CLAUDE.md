...
- **Uptime is COMPUTED by AIWatch, not copied** (#1006) — from the provider's own published records, with one window and one formula for every service: Atlassian → `parseUptimeData` (per-day outage seconds); incident.io → `computeIncidentIoUptime` (`component_impacts` intervals). Weights per `/methodology`: full outage 1.0, partial/degraded 0.3, announced maintenance excluded. incident.io's published `component_uptimes[].uptime` aggregate is NOT a 30-day figure and differs in definition page to page (LangSmith's tracked ~90d and ranked it `fair` on 60-day-old outages; OpenAI's excludes degraded/partial), so copying it made the ranking compare incomparable numbers. Our figure can therefore differ from the % on a provider's own page — by design, and disclosed. A provider whose records don't reach back 30 days (a status-page migration resets the component clock) reports `uptimeWindowDays` and the UI states the real window.
