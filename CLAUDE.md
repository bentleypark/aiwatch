...
worker/
  src/
    ...
    parsers/    # Platform-specific parsers (statuspage, incident-io, gcloud, aistudio, instatus, betterstack, aws, openrouter-uptime)
                ...
                # openrouter-uptime.ts (#371): xAI-only supplementary signal. status.x.ai's per-endpoint success rates are Cloudflare-blocked to non-browser clients (only feed.xml is whitelisted, incidents-only), so this reads OpenRouter's `uptime_last_30m` per routing endpoint instead — averages the `xAI`-provider value across a curated set of flagship Grok model slugs (XAI_OPENROUTER_MODEL_SLUGS; update on xAI model renames). Returns null on any failure → the detail-page line is hidden. Exposed via `GET /api/openrouter-uptime/xai` (5-min edge cache); NOT in KV, NOT in `/api/status`, NOT in the AIWatch Score — informational only. Frontend shows it as a separate `OpenRouterUptime` section on the xAI ServiceDetails page with the 30-min-rolling / routing-traffic / not-official caveats in the copy + a tooltip. The worker gates with `gated: true` when the `openrouter` service is itself non-operational (its uptime then reflects OpenRouter routing, not xAI) and the frontend flags that.
```
...
- Status polling proxy: `worker/` directory (monorepo), Cloudflare Workers
  ...
  - Endpoints: `GET /api/status`, `GET /api/status/cached` (KV-only, includes probe24h, for SSR + initial load), `GET /api/uptime?days=30`, `GET /api/probe/history?days=30` (daily probe RTT summaries, 90d max), `GET /api/openrouter-uptime/xai` (#371 — OpenRouter-measured xAI availability for the xAI ServiceDetails informational line; `{ available, uptimePct, sampleCount, measuredAt, gated, modelSlugs }`; 5-min edge cache; `gated:true` when the `openrouter` service is itself non-operational; not in KV/`/api/status`/Score), `GET /api/report?month=YYYY-MM` (monthly archive JSON, permanent), `POST /api/alert`, `POST /api/admin/analyze` (operator Sonnet override, `X-Admin-Key` required, sets `sticky: true` so cron doesn't auto-replace), `POST /api/admin/rebuild-archive` (operator regenerate of `archive:monthly:{YYYY-MM}` after a bug-fix deploy, `X-Admin-Key` required), `POST /api/internal/edge-fallback` (Bearer-authenticated `EDGE_ALERT_TOKEN`; called by Vercel Edge Functions on degraded fallback render, dedups via `alerted:edge-fallback:*` and fires Discord — #378), `GET /badge/:serviceId`, `GET /api/og` (dynamic OG image PNG), `GET /api/v1/status`
