...
worker/
  src/
    ...
    alerts.ts   # Alert detection logic (buildIncidentAlerts, buildServiceAlerts, formatDetectionLead, buildRegionHint, buildTweetDraft). buildRegionHint (#422 Phase 2) reuses the Edge region-status port (api/is-down/region-status.ts) — imported, not re-copied — to append a "📍 Try region: <label>" line to new-incident Discord embeds for region-aware services with a region-specific partial outage. buildTweetDraft (#348 Phase 1.5) returns an outage/recovery tweet + X compose (Web Intent) link for the 6 Claude/OpenAI-family services (claude/openai/claudeai/chatgpt/claudecode/codex; slugs pinned by tweet-draft-slug-sync.test.ts) — appended to the operator embed only (see "Two Discord alert paths"), never the per-user relay
    ...
    detection-lead-log.ts # Detection Lead audit log — per-day KV array (#256), tagged AppendResult, 24h sliding window for daily summary. Also `classifyLead` + `detection:lead:diag:{date}` counter (#464): measures why leads are/aren't recorded (no_detected/negative/below_min/in_window/above_max × probe/non-probe). **#464 redefinition**: diag data showed `in_window`=0 — status-page polling is structurally later than the official publish, so the "faster than official" claim is retired. Honest framing = MTTD ("alerted within ~5-min polling cycle", via `formatLeadDiagSection`) + RTT degradation detection (`classifyDegradation` → `probe-degradation:daily`/`:nostatus` counters, surfaced by `formatDegradationSection` in daily-summary — degradations status pages don't report). Aggregate "average lead" gated by `MIN_LEAD_SAMPLE_SIZE` (consumer-side `canPresentLeadAverage` in monthly-archive) so a marketing claim never rests on ~0 samples; per-event early-RTT signals stay shown when genuine
...
<<<<<<< HEAD
  ...
  - **Two Discord alert paths** (#467, #475): (1) **operator** — the cron above posts to the single `env.DISCORD_WEBHOOK_URL` secret (always on for the operator). (2) **per-user** — a visitor's own Discord webhook (Settings → Alerts), delivered **browser-side** by `src/utils/webhookAlerts.js` (`runWebhookAlerts`, from `usePolling` each live poll — 60s visible / 5min hidden — only while a tab is open). **#475 single source of truth**: the cron appends every embed it sends the operator to a rolling KV feed (`alert:feed:recent`, built in `worker/src/alert-feed.ts`), surfaced as the `alertFeed` field on `/api/status` (+`/cached`); the browser is a **dumb relay** — it applies the per-user filter and POSTs the worker's embed *verbatim*, so operator/user alerts are **byte-identical** (Detection Lead, AI analysis, grouping, fallback, region all included) and duplicate suppression — incl. the #473 cross-poll status↔incident race — lives server-side and can't reappear in JS. Consequence: user alerts fire at **cron cadence (≤~5min, same time as the operator)**, not the old browser-computed 60s. The two target *different* webhooks (no cross-source dupe). The server stores only a SHA-256 hash of the user URL (`webhook:reg:{hash}`, count-only), so the worker provides payloads but the browser does the POST; `/api/alert` is a CORS/SSRF-guarded Discord proxy (Discord hosts only, #468). Slack subscriptions use Slack's native `/feed` RSS app. Per-user filter honors `alertCondition` (`down`/`all`, #470) + `alertTarget` (`all`/`custom` picker) + `alertIncidents` (default on). History: #467 restored delivery; #473/#474 chased operator-format parity in JS until #475 moved formatting server-side (#474's `getGroupedFallbacks`/`sanitizeForDiscord`/grouping now run once, in the worker). **One operator-only exception (#348 Phase 1.5)**: for the 6 Claude/OpenAI-family services (claude/openai/claudeai/chatgpt/claudecode/codex) the operator embed carries an extra `🐦 TWEET DRAFT` section (an outage/recovery tweet + a prefilled X compose link from `buildTweetDraft`). It's a manual-assist operator action — appended to the `sendDiscordAlert` description *after* `buildFeedEntry` runs on the clean description, so it is **never** in the per-user relay. Full X API auto-posting remains #348 Phase 2 (not built)
=======
  - **Cron Trigger**: `*/5 * * * *` — alert detection runs every 5 minutes via scheduled handler (not per-request). Uses KV ID-based dedup (`alerted:new/res:` keys 7d TTL, `alerted:down/degraded/recovered:` keys 2h TTL). Fallback recommendations only included when service status is degraded/down (not operational). AI analysis runs inline with 8s timeout — Gemma 4 26B (Workers AI) primary, Sonnet (AI Gateway) fallback — results stored in `ai:analysis:{svcId}:{incId}` (1h TTL, per-incident). Daily alert counts tracked in `alert:count:{date}` for Daily Summary
  - **Discord alert delivery** (operator + per-user paths): both the operator webhook and confirmed per-user webhooks are delivered **server-side** by the `*/5` cron — the full annotated description (single-source-of-truth `alert:feed:recent`, per-user filters, the #486 channel-control double opt-in with AES-GCM-encrypted URL storage, and the PR3 cutover that retired the browser relay) lives in **[docs/reference/discord-alert-paths.md](docs/reference/discord-alert-paths.md)**.
>>>>>>> ad0361a
