...
6. **Fix → re-test → re-review, auto-loop** until 0 Critical/Important (Suggestions-only = converged). Gate each finding on whether it arrived with a reproduction or failing check; for those that did, never cap the rounds. That rule and what to carry into each review prompt are in `ship-issue` steps 5-6 (#1097/#1124/#1245)
...
worker/src/          # Cloudflare Worker: status polling, KV cache, cron, alerts, AI analysis
  ...
  parsers/           # statuspage, incident-io, gcloud, aistudio, instatus, betterstack, aws, flashduty, rootly
