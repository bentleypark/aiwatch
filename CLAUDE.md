...
- Status polling proxy: `worker/` directory (monorepo), Cloudflare Workers
  ...
  - **Discord alert delivery** (operator + per-user paths, #467/#475, and the #486 server-side transition): the full annotated description — browser-relay single-source-of-truth, per-user filters, and the in-progress channel-control double opt-in (AES-GCM-encrypted URL storage, PR1/PR2 merged, PR3 wires the cron fan-out + Privacy policy) — lives in **[docs/reference/discord-alert-paths.md](docs/reference/discord-alert-paths.md)**. ⚠️ Do not deploy the worker for #486 until PR3 is ready (see that doc).
