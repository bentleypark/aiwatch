...
Every fire is logged to `.claude/hook-audit.jsonl` (gitignored). `node scripts/hook-audit-summary.mjs` summarizes (by hook × decision, last-7-days, per-day trend, recent entries) — review periodically; if the git gate keeps getting ignored, escalate it to a hard block (`permissionDecision: "deny"`). New `.claude/settings.json` only takes effect after `/hooks` is opened once or a restart (the settings watcher only watches dirs that had a settings file at session start).
