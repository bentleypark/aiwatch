...
- **CSP (#482)**: violations land at the `api/csp-report.ts` Edge sink (Vercel logs). **Edge SSR pages ENFORCE their own per-response CSP** (#823/#828/#829 + Phase-3 flip): each handler sets `Content-Security-Policy` itself, split by cacheability — **nonce** (`api/_shared/csp-nonce.ts`, `Cache-Control: no-store`) for low-traffic badges/methodology/intro, **content hash** (`api/_shared/csp-hash.ts` `cspForHtml`) for high-traffic **is-down** so it keeps its `s-maxage=60` edge cache (a content hash survives caching; a random nonce would be reused across all cached-page visitors). Every inline `on*=` handler was first refactored to a delegated listener. **The SPA stays `Content-Security-Policy-Report-Only`** via `vercel.json` `source:"/(.*)"` (that global header also co-applies on the Edge routes but Report-Only never blocks — the Edge's own enforcing policy gates those pages). **Remaining #482 work**: enforce the SPA — `index.html` has inline `<link onload/onerror>` + a `<script>` to hash/refactor first, then flip vercel.json to enforcing AND scope its `/(.*)` to exclude the Edge routes (so two enforcing policies don't intersect-block). Full policy/rationale/roadmap: **[docs/reference/reference-csp.md](docs/reference/reference-csp.md)** — read before changing the policy or adding an inline script/external origin
